...
# general options
node_name="node5" # docker container prefix
node_user="node5" # run as username
node_uid="$(id -u $node_user)"
extip="135.181.231.117" # public ip of the node

# images
el_image="pk910/ephemery-geth:latest"
bn_image="pk910/ephemery-lodestar:latest"
vc_image="pk910/ephemery-lodestar:latest"

# datadirs
el_datadir="/data/node5/execution"
bn_datadir="/data/node5/beacon"
vc_datadir="/data/node5/validator"
jwtsecret_file="/data/node5/jwtsecret"
...
port_offset=20  # increase all ports by this offset
...
start_bn() {
  ...
  docker run -d --restart unless-stopped --name=$node_name-bn \
    ...
    beacon \
    --dataDir=/data \
    --discv5 --nat=true --network.connectToDiscv5Bootnodes \
    --listenAddress=0.0.0.0 --enr.ip=$extip --port=$p2p_port --enr.tcp=$p2p_port --enr.udp=$p2p_port \
    --rest --rest.address=0.0.0.0 --rest.namespace="*" --rest.port=$rpc_port \
    --execution.urls=http://172.17.0.1:$engine_port --jwt-secret=/execution-auth.jwt \
...
start_vc() {
  ...
  docker run -d --restart unless-stopped --name=$node_name-vc \
    ...
    validator \
    --dataDir=/data \
    --beaconNodes=http://172.17.0.1:$rpc_port \
    --keystoresDir=/data/keys --secretsDir=/data/secrets \
...
copy_vc_keys() {
  ...
  cp $keystores/lodestar-secrets -r $vc_datadir/secrets
